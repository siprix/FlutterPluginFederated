...
public class SiprixVoipSdkPlugin: NSObject, FlutterPlugin {
  ...
  func handleModuleInitialize(_ args : ArgsMap, result: @escaping FlutterResult) {
        ...
        if let license = args["license"] as? String {
           iniData.license = license }

        if let brandName = args["brandName"] as? String {
           iniData.brandName = brandName }
        
        if let logLevelFile = args["logLevelFile"] as? Int {
           iniData.logLevelFile = NSNumber(value: logLevelFile) }
        
        if let logLevelIde = args["logLevelIde"] as? Int {
           iniData.logLevelIde = NSNumber(value: logLevelIde) }
        
        if let rtpStartPort = args["rtpStartPort"] as? Int {
           iniData.rtpStartPort = NSNumber(value: rtpStartPort) }
        
        if let tlsVerifyServer = args["tlsVerifyServer"] as? Bool {
           iniData.tlsVerifyServer = NSNumber(value: tlsVerifyServer) }
        
        if let singleCallMode = args["singleCallMode"] as? Bool {
           iniData.singleCallMode = NSNumber(value: singleCallMode) }
        
        if let shareUdpTransport = args["shareUdpTransport"] as? Bool {
           iniData.shareUdpTransport = NSNumber(value: shareUdpTransport) }

        if let unregOnDestroy = args["unregOnDestroy"] as? Bool {
           iniData.unregOnDestroy = NSNumber(value: unregOnDestroy) }

        if let useDnsSrv = args["useDnsSrv"] as? Bool {
           iniData.useDnsSrv = NSNumber(value: useDnsSrv) }

        if let dnsServers = args["dnsServers"] as? String {
           iniData.dnsServers = [dnsServers] }

        if let recordStereo = args["recordStereo"] as? Bool {
           iniData.recordStereo = NSNumber(value: recordStereo) }

        if let enableVideoCall = args["enableVideoCall"] as? Bool {
           iniData.enableVideoCall = NSNumber(value: enableVideoCall) }

        if let transpForceIPv4 = args["transpForceIPv4"] as? Bool {
           iniData.transpForceIPv4 = NSNumber(value: transpForceIPv4) }

        if let enableAes128Sha32 = args["enableAes128Sha32"] as? Bool {
           iniData.enableAes128Sha32 = NSNumber(value: enableAes128Sha32) }

        if let enableVUmeter = args["enableVUmeter"] as? Bool {
           iniData.enableVUmeter = NSNumber(value: enableVUmeter) }
    ...
    func parseAccData(_ args : ArgsMap) -> SiprixAccData {
        ...
        if let sipServer = args["sipServer"] as? String {
           accData.sipServer = sipServer }

        if let sipExtension = args["sipExtension"] as? String {
           accData.sipExtension = sipExtension }
        
        if let sipPassword = args["sipPassword"] as? String {
           accData.sipPassword = sipPassword }
        
        if let sipAuthId = args["sipAuthId"] as? String {
           accData.sipAuthId = sipAuthId }
        
        if let sipProxy = args["sipProxy"] as? String {
           accData.sipProxy = sipProxy }
        
        if let displName = args["displName"] as? String {
           accData.displName = displName }

        if let userAgent = args["userAgent"] as? String {
           accData.userAgent = userAgent }
        
        if let expireTime = args["expireTime"] as? Int {
           accData.expireTime = NSNumber(value:expireTime) }
      
        if let transport = args["transport"] as? Int {
           accData.transport = SipTransport(rawValue: transport)! }

        if let port = args["port"] as? Int {
           accData.port = NSNumber(value:port) }

        if let tlsCaCertPath = args["tlsCaCertPath"] as? String {
           accData.tlsCaCertPath = tlsCaCertPath }

        if let tlsUseSipScheme = args["tlsUseSipScheme"] as? Bool {
           accData.tlsUseSipScheme = NSNumber(value:tlsUseSipScheme) }

        if let tlsClientCertPath = args["tlsClientCertPath"] as? String {
           accData.tlsClientCertPath = tlsClientCertPath }

        if let tlsClientKeyPath = args["tlsClientKeyPath"] as? String {
           accData.tlsClientKeyPath = tlsClientKeyPath }

        if let tlsClientKeyPassword = args["tlsClientKeyPassword"] as? String {
           accData.tlsClientKeyPassword = tlsClientKeyPassword }

        if let preferIPv6 = args["preferIPv6"] as? Bool {
           accData.transpPreferIPv6 = NSNumber(value:preferIPv6) }

        if let rtcpMuxEnabled = args["rtcpMuxEnabled"] as? Bool {
           accData.rtcpMuxEnabled = NSNumber(value:rtcpMuxEnabled) }

        if let iceEnabled = args["iceEnabled"] as? Bool {
           accData.iceEnabled = NSNumber(value:iceEnabled) }

        if let instanceId = args["instanceId"] as? String {
           accData.instanceId = instanceId }
        
        if let ringTonePath = args["ringTonePath"] as? String {
           accData.ringTonePath = ringTonePath }
        ...
        if let verifyIncomingCall = args["verifyIncomingCall"] as? Bool {
           accData.verifyIncomingCall = NSNumber(value: verifyIncomingCall) }

        if let forceSipProxy = args["forceSipProxy"] as? Bool {
           accData.forceSipProxy = NSNumber(value: forceSipProxy) }

        if let secureMedia = args["secureMedia"] as? Int {
           accData.secureMedia = NSNumber(value:secureMedia) }

        if let upgradeToVideo = args["upgradeToVideo"] as? Int {
           accData.upgradeToVideo = NSNumber(value:upgradeToVideo) }

        if let xheaders = args["xheaders"] as? Dictionary<AnyHashable,Any> {
           accData.xheaders = xheaders }

        if let stunServer = args["stunServer"] as? String {
           accData.stunServer = stunServer }

        if let turnServer = args["turnServer"] as? String {
           accData.turnServer = turnServer }

        if let turnUser = args["turnUser"] as? String {
           accData.turnUser = turnUser }

        if let turnPassword = args["turnPassword"] as? String {
           accData.turnPassword = turnPassword }
        
        if let xContactUriParams = args["xContactUriParams"] as? Dictionary<AnyHashable,Any> {
           accData.xContactUriParams = xContactUriParams }
        
        if let aCodecs = args["aCodecs"] as? [Int] {
           accData.aCodecs = aCodecs }
        
        if let vCodecs = args["vCodecs"] as? [Int] {
           accData.vCodecs = vCodecs }
    ...
    func handleCallInvite(_ args : ArgsMap, result: @escaping FlutterResult) {
        ...
        if let toExt = args["extension"] as? String {
            destData.toExt = toExt }
        
        if let fromAccId = args[kArgAccId] as? Int {
            destData.fromAccId = Int32(fromAccId) }
        
        if let inviteTimeout = args["inviteTimeout"] as? Int {
            destData.inviteTimeoutSec = NSNumber(value:inviteTimeout) }
        
        if let withVideo = args[kArgWithVideo] as? Bool {
            destData.withVideo = NSNumber(value: withVideo) }
        
        if let xheaders = args["xheaders"] as? Dictionary<AnyHashable,Any> {
            destData.xheaders = xheaders }
     
        if let displName = args["displName"] as? String {
            destData.displName = displName }
    ...
    func handleSubscriptionAdd(_ args : ArgsMap, result: @escaping FlutterResult) {
        ...
        if let toExt = args["extension"] as? String {
           subscrData.toExt = toExt }
        
        if let fromAccId = args[kArgAccId] as? Int {
           subscrData.fromAccId = Int32(fromAccId) }

        if let expireTime = args["expireTime"] as? Int {
           subscrData.expireTime = NSNumber(value:expireTime) }
        
        if let mimeSubType = args["mimeSubType"] as? String {
           subscrData.mimeSubtype = mimeSubType }

        if let eventType = args["eventType"] as? String {
           subscrData.eventType = eventType }

        if let body = args["body"] as? String {
           subscrData.body = body }
    ...
    func handleDvcSetVideoParams(_ args : ArgsMap, result: @escaping FlutterResult) {
        ...
        if let noCameraImgPath = args["noCameraImgPath"] as? String {
           vdoData.noCameraImgPath = noCameraImgPath }
        
        if let framerateFps = args["framerateFps"] as? Int {
           vdoData.framerateFps = NSNumber(value: framerateFps) }
        
        if let bitrateKbps = args["bitrateKbps"] as? Int {
           vdoData.bitrateKbps = NSNumber(value: bitrateKbps) }
        
        if let height = args["height"] as? Int {
           vdoData.height = NSNumber(value: height) }
        
        if let width = args["width"] as? Int {
           vdoData.width = NSNumber(value: width) }
